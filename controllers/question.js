...
const app = {
  getQuizandQuestion: async function () {
    ...
    // //   xu li hien thi danh sach top 10
    // this.renderTop10(id);
  },
  ...
  checkAnswers: async function (listAnswerSubmit) {
    ...
    // console.log(searchParam);
    if (searchParam.has("id")) {
      var quizId = searchParam.get("id");
    }
    this.handleAddUser(countRight, quizId);
    // //   xu li hien thi danh sach top 10
    console.log(quizId);
    this.renderTop10(quizId);
    //console.log(tbody);
  },
  ...
  handleAddUser: async function (countRight, quizId) {
    var username = prompt("Vui long nhap ten cua ban: ");
    console.log(username);
    if (username) {
      // lay time now
      const currentTime = new Date();
      const day = currentTime.getDate(); // Lấy ngày
      const month = currentTime.getMonth() + 1; // Lấy tháng (tháng bắt đầu từ 0 nên cần +1)
      const year = currentTime.getFullYear(); // Lấy năm
      const hours = currentTime.getHours(); // Lấy giờ
      const minutes = currentTime.getMinutes(); // Lấy phút
      const seconds = currentTime.getSeconds(); // Lấy giây
      ...
      // luu vao db.json
      await addUser(data);
      //   alert("them dl thanh cong");
    }
  },
  renderTop10: async function (id) {
    console.log(id);
    window.location.href = `top10.html?quizId=${id}`;
  },
